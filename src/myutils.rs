...
/// Clears and releases the capacity of an I/O scratch buffer.
#[track_caller]
pub fn free_buffer(buffer: &mut Vec<u8>) {
    buffer.clear();
...
/// Lists `dir_name` entries (sorted, including `.` and `..`).
#[track_caller]
...
/// 64-bit variant: seeks to `pos` and reads exactly `bytes`.
#[track_caller]
...
pub fn read_all_stdio_file(f: &mut std::fs::File) -> Vec<byte> {
    ...
    assert_eq!(file_size as uint64, file_size64);
...
/// Reads the entire (open) file in 1 GB chunks.
#[track_caller]
...
/// Opens `file_name`, asserts the size fits in `usize`, and reads it fully.
#[track_caller]
pub fn read_all_stdio_file32(file_name: &str) -> Vec<byte> {
    let mut f = open_stdio_file(file_name);
    let saved_pos = get_stdio_file_pos64(&mut f);
    let file_size64 = get_stdio_file_size64(&mut f);

    if usize::try_from(file_size64).is_err() {
        panic!("ReadAllStdioFile (32-bit): file too big");
    }

    let file_size = file_size64 as usize;
    set_stdio_file_pos(&mut f, 0);
    let mut buffer = vec![0; file_size];
    std::io::Read::read_exact(&mut f, &mut buffer).expect("ReadStdioFile failed");
    set_stdio_file_pos64(&mut f, saved_pos);
    buffer
}
...
/// Returns the total size of `f` as a 32-bit value (panics if it overflows).
#[track_caller]
...
/// Returns the current resident-set-size in bytes (variant 1).
#[track_caller]
...
/// Returns current RSS bytes by reading `/proc/self/status`.
#[track_caller]
...
/// Returns RSS bytes on Windows via the process API.
#[track_caller]
pub fn get_mem_use_bytes_l1089() -> f64 {
    0.0
}

/// Returns directory entries in `dir_name` (excluding `.`/`..`).
#[track_caller]
...
/// Installs the default progress callback (no-op stub).
#[track_caller]
pub fn set_pcb() {
    let mut state = PROGRESS_STATE.lock().unwrap();
    state.desc = default_pcb().to_string();
}
...
pub fn progress_file_init(f: &mut std::fs::File, msg: Option<&str>) -> String {
    ...
    state.desc = state.prog_file_msg.clone();
    state.index = 0;
    state.count = 1000;
    format!("{} {}", pct_str(1.0, 1000.0), state.prog_file_msg)
}
...
pub fn progress_callback(i: uint, n: uint) -> String {
    ...
    format!(" {} {}", pct_str((i + 1) as f64, n as f64), default_pcb())
}
...
pub fn try_flag_opt(opt_name: &str) -> bool {
    FLAG_OPT_NAMES.contains(&opt_name)
}
...
pub fn try_uns_opt(opt_name: &str, value: &str) -> Option<uint> {
    if UNS_OPT_NAMES.contains(&opt_name) {
        Some(str_to_uint_l1278(value, false))
    } else {
        None
    }
}
...
pub fn try_float_opt(opt_name: &str, value: &str) -> Option<f64> {
    if FLT_OPT_NAMES.contains(&opt_name) {
        Some(str_to_float_l1209(value, false))
    } else {
        None
    }
}
