...
/// Backward HMM pass over Mega profiles: same recurrence as `calc_bwd_flat_l10`
/// but with emission scores looked up from the profile rather than raw bytes.
#[track_caller]
pub fn mega_calc_bwd_flat_mega(profile_x: &[Vec<byte>], profile_y: &[Vec<byte>], flat: &mut [f32]) {
    ...
    for i in (0..=i_lx).rev() {
        ...
        for j in (0..=i_ly).rev() {
            ...
            if i < i_lx && j < i_ly {
                let next_m = flat[base_i1_j1 as usize + HMMSTATE_M as usize] + emit_xy;
                ...
                if i > 0 && j > 0 {
                    ...
                    flat[b + HMMSTATE_M as usize] = log_add_hack(
                        m_m,
                ...
                if i > 0 {
                    flat[b + HMMSTATE_IX as usize] = log_add_hack(t_ii + next_ix, t_im + next_m);
                    flat[b + HMMSTATE_JX as usize] = log_add_hack(t_jj + next_jx, t_jm + next_m);
                } else {
                ...
                if j > 0 {
                    flat[b + HMMSTATE_IY as usize] = log_add_hack(t_ii + next_iy, t_im + next_m);
                    flat[b + HMMSTATE_JY as usize] = log_add_hack(t_jj + next_jy, t_jm + next_m);
                } else {
